...
# Not bidirectional
class TDSConv_LSTM_CTC_Hybrid(pl.LightningModule):
    NUM_BANDS: ClassVar[int] = 2
    ELECTRODE_CHANNELS: ClassVar[int] = 16
...
class TDSConv_GRU_CTC_Hybrid(pl.LightningModule):
    NUM_BANDS: ClassVar[int] = 2
    ELECTRODE_CHANNELS: ClassVar[int] = 16
...
class TDSConv_Transformer_CTC_Hybrid(pl.LightningModule):
    NUM_BANDS: ClassVar[int] = 2
    ELECTRODE_CHANNELS: ClassVar[int] = 16
    ...
    def __init__(
        ...
        encoder_layer = nn.TransformerEncoder(d_model=transformer_hidden_size,
                                            ...
                                            dropout=0)
        # Dropout set to 0 for consistent training against LSTM and GRU
        # 2048 is default, but 1024 2-4x larger than num_features = 256
        ...
        self.classifier = nn.Sequential(
            # (T, N, num_classes)
            nn.Linear(transformer_hidden_size, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
